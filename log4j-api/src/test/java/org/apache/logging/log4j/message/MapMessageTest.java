...
public class MapMessageTest {
	...
	public void testRemoveUsesDeepToString() {
		final String key = "key";
		StringBuilderFormattable type = Mockito.mock(StringBuilderFormattable.class);
		Mockito.when(type.toString()).thenReturn("toString");
		Mockito.doAnswer(invo -> {
			((StringBuilder) invo.getArgument(0)).append("formatTo");
			return null;
		}).when(type).formatTo(Mockito.any());
		final ObjectMapMessage msg = new ObjectMapMessage().with(key, type);
