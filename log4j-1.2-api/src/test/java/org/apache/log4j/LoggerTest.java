...
public class LoggerTest {
	...
	/**
	 * Add an appender and see if it can be retrieved. Skipping this test as the
	 * Appender interface isn't compatible with legacy Log4j. public void
	 * testAppender1() { logger = Logger.getLogger("test"); a1 = new
	 * ListAppender("testAppender1"); logger.addAppender(a1);
	 * 
	 * Enumeration enumeration = logger.getAllAppenders(); Appender aHat =
	 * (Appender) enumeration.nextElement(); assertEquals(a1, aHat); }
	 */

	/**
	 * Add an appender X, Y, remove X and check if Y is the only remaining
	 * appender. Skipping this test as the Appender interface isn't compatible
	 * with legacy Log4j. public void testAppender2() { a1 = new FileAppender();
	 * a1.setName("testAppender2.1"); a2 = new FileAppender();
	 * a2.setName("testAppender2.2");
	 * 
	 * logger = Logger.getLogger("test"); logger.addAppender(a1);
	 * logger.addAppender(a2); logger.removeAppender("testAppender2.1");
	 * Enumeration enumeration = logger.getAllAppenders(); Appender aHat =
	 * (Appender) enumeration.nextElement(); assertEquals(a2, aHat);
	...
	/*
	 * Don't support getLoggerRepository public void testDisable1() {
	 * CountingAppender caRoot = new CountingAppender(); Logger root =
	 * Logger.getRootLogger(); root.getLogger().addAppender(caRoot);
	 * 
	 * LoggerRepository h = LogManager.getLoggerRepository();
	 * //h.disableDebug(); h.setThreshold((Level) Level.INFO);
	 * assertEquals(caRoot.counter, 0);
	 * 
	 * root.debug(MSG); assertEquals(caRoot.counter, 0); root.info(MSG);
	 * assertEquals(caRoot.counter, 1); root.log(Level.WARN, MSG);
	 * assertEquals(caRoot.counter, 2); root.warn(MSG);
	 * assertEquals(caRoot.counter, 3);
	 * 
	 * //h.disableInfo(); h.setThreshold((Level) Level.WARN); root.debug(MSG);
	 * assertEquals(caRoot.counter, 3); root.info(MSG);
	 * assertEquals(caRoot.counter, 3); root.log(Level.WARN, MSG);
	 * assertEquals(caRoot.counter, 4); root.error(MSG);
	 * assertEquals(caRoot.counter, 5); root.log(Level.ERROR, MSG);
	 * assertEquals(caRoot.counter, 6);
	 * 
	 * //h.disableAll(); h.setThreshold(Level.OFF); root.debug(MSG);
	 * assertEquals(caRoot.counter, 6); root.info(MSG);
	 * assertEquals(caRoot.counter, 6); root.log(Level.WARN, MSG);
	 * assertEquals(caRoot.counter, 6); root.error(MSG);
	 * assertEquals(caRoot.counter, 6); root.log(Level.FATAL, MSG);
	 * assertEquals(caRoot.counter, 6); root.log(Level.FATAL, MSG);
	 * assertEquals(caRoot.counter, 6);
	 * 
	 * //h.disable(Level.FATAL); h.setThreshold(Level.OFF); root.debug(MSG);
	 * assertEquals(caRoot.counter, 6); root.info(MSG);
	 * assertEquals(caRoot.counter, 6); root.log(Level.WARN, MSG);
	 * assertEquals(caRoot.counter, 6); root.error(MSG);
	 * assertEquals(caRoot.counter, 6); root.log(Level.ERROR, MSG);
	 * assertEquals(caRoot.counter, 6); root.log(Level.FATAL, MSG);
	 * assertEquals(caRoot.counter, 6); }
	 */
	...
	/*
	 * Don't support hierarchy public void testHierarchy1() { Hierarchy h = new
	 * Hierarchy(new RootLogger((Level) Level.ERROR)); Logger a0 =
	 * h.getLogger("a"); assertEquals("a", a0.getName());
	 * assertNull(a0.getLevel()); assertSame(Level.ERROR,
	 * a0.getEffectiveLevel());
	 * 
	 * Logger a1 = h.getLogger("a"); assertSame(a0, a1); }
	 */
